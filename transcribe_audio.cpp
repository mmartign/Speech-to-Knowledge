...
namespace Constants {
    ...
    constexpr double ADAPTIVE_UPDATE_MAX_RMS_FRACTION = 0.35;
...
private:
    ...
    void process_chunk(const std::vector<int16_t>& current_chunk) {
        ...
        const bool is_speech = sum_squares > (threshold_squared * current_chunk.size());
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--energy_threshold" && i + 1 < argc) {
            ...
            std::cout
                ...
                << "  --adaptive_silence_fraction <float>  Silence RMS fraction used before sending to Whisper. Default: 0.20\n"
...
namespace {
    ...
    void run_ffmpeg_extract(const std::string& input, const std::string& output, int target_sample_rate) {
        ...
        const std::string cmd =
            quote_windows_arg("ffmpeg") + " -y -i " + quote_windows_arg(input) +
            " -vn -ac 1 -ar " + std::to_string(target_sample_rate) +
            " -sample_fmt s16 " + quote_windows_arg(output);
        ...
        if (!CreateProcessA(nullptr,
                            ...
                            &si,
                            &pi)) {
            throw AudioException("Failed to start ffmpeg process.");
        }
        ...
        if (exit_code != 0) {
            throw AudioException("ffmpeg failed to extract audio from media file.");
        }
#else
        pid_t pid = fork();
        if (pid < 0) {
            throw AudioException("Failed to fork ffmpeg process.");
        }
        if (pid == 0) {
            execlp("ffmpeg",
                   "ffmpeg",
                   ...
                   "1",
                   "-ar",
                   std::to_string(target_sample_rate).c_str(),
                   "-sample_fmt",
                   "s16",
                   output.c_str(),
        ...
        if (waitpid(pid, &status, 0) < 0) {
            throw AudioException("Failed waiting for ffmpeg process.");
        }
...
bool load_wav_mono_16(const std::string& path, int& sample_rate_out, std::vector<int16_t>& samples_out) {
    ...
    f.read(tag.data(), 4);
    if (f.gcount() != 4 || std::string(tag.data(), 4) != "RIFF") {
        return false;
    }

    (void)read_u32_le(f);
    f.read(tag.data(), 4);
    if (f.gcount() != 4 || std::string(tag.data(), 4) != "WAVE") {
        return false;
    }
    ...
    while (f && !data_found) {
        ...
        const std::string chunk_id(tag.data(), 4);

        if (chunk_id == "fmt ") {
            const uint16_t audio_format = read_u16_le(f);
...
std::vector<int16_t> load_raw_pcm_16(const std::string& path) {
    ...
    std::vector<int16_t> data(static_cast<size_t>(sz / static_cast<std::streamsize>(sizeof(int16_t))));
    f.read(reinterpret_cast<char*>(data.data()), sz);
    return f ? data : std::vector<int16_t>{};
}
...
std::string transcode_media_to_wav(const std::string& media_path, int target_sample_rate) {
    ...
    const fs::path tmp = fs::temp_directory_path() / ("stkw_media_" + std::to_string(ts) + ".wav");

    run_ffmpeg_extract(media_path, tmp.string(), target_sample_rate);

    if (!fs::exists(tmp)) {
        throw AudioException("ffmpeg did not produce the expected WAV output.");
    }
    return tmp.string();
}
}  // namespace FileAudio
...
std::string format_datetime(const std::chrono::time_point<std::chrono::system_clock, Duration>& tp) {
    ...
    const std::time_t tt = std::chrono::system_clock::to_time_t(tp_sys);
    std::tm tm{};
#if defined(_WIN32)
    localtime_s(&tm, &tt);
#else
